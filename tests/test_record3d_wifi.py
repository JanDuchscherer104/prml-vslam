...
def test_render_record3d_wifi_viewer_mounts_with_stable_key(monkeypatch) -> None:
...
def test_record3d_wifi_assets_cover_webrtc_signaling_and_cleanup() -> None:
    ...
    assert "Chrome and Safari" in RECORD3D_WIFI_COMPONENT_HTML
    assert "lower fidelity than the USB Python integration" in RECORD3D_WIFI_COMPONENT_HTML
    ...
    assert "inv_dist_std" in RECORD3D_WIFI_COMPONENT_HTML
    ...
    assert "getMetadata" in RECORD3D_WIFI_COMPONENT_JS
    assert "RTCPeerConnection" in RECORD3D_WIFI_COMPONENT_JS
    assert "onicecandidate" in RECORD3D_WIFI_COMPONENT_JS
    assert "ontrack" in RECORD3D_WIFI_COMPONENT_JS
    assert "requestAnimationFrame" in RECORD3D_WIFI_COMPONENT_JS
    assert "getImageData" in RECORD3D_WIFI_COMPONENT_JS
    assert "putImageData" in RECORD3D_WIFI_COMPONENT_JS
    assert "rgbToHue" in RECORD3D_WIFI_COMPONENT_JS
    assert "equalizeHistogram" in RECORD3D_WIFI_COMPONENT_JS
    assert "equalizeDepthHistogram" in RECORD3D_WIFI_COMPONENT_JS
    assert "showInvDistStd" in RECORD3D_WIFI_COMPONENT_JS
    assert "close()" in RECORD3D_WIFI_COMPONENT_JS
    assert "beforeunload" in RECORD3D_WIFI_COMPONENT_JS
    assert "return () =>" in RECORD3D_WIFI_COMPONENT_JS
    assert "403" in RECORD3D_WIFI_COMPONENT_JS
