...
- The stable lessons from the Rerun regression work are:
  ...
  - preserve upstream/native ViSTA world orientation on the repo-owned sink path; use an explicit neutral root `Transform3D()` plus `ViewCoordinates.RDF` at `world` when the scene root should be declared, but do not add a viewer-only basis transform that rotates the data
...
- The current sink semantics use a split viewer tree of `world/live/source` for source imagery plus `world/slam/vista_slam_world/live/{tracking,model}` and `world/slam/vista_slam_world/keyframes/{cameras,points}/<id>` for ViSTA outputs. Camera-local pointmaps live at `world/slam/vista_slam_world/keyframes/points/<id>/points` beneath a posed point-history parent while keyed camera frusta live under `world/slam/vista_slam_world/keyframes/cameras/<id>`.
- The default 3D blueprint should render keyed-history `world/slam/vista_slam_world/keyframes/points/<id>/points` as the accumulated map and hide `world/slam/vista_slam_world/live/model/points` by default; the live/model point cloud remains a latest/debug-only surface.
