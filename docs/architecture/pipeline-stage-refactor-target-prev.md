...
<!--
TODO: Collapse StageOutput and StageOutcome

TODO: Remove StageCatalog From Target Architecture
CONTEXT:
## Summary
Yes, it is possible to remove the standalone `StageRegistry` / `StageCatalog` concept from the target architecture. The cleaner target is: `RunConfig` owns a typed `StageBundle`, and each `StageConfig` owns its own side-effect-free planning metadata. `RunConfig.compile_plan()` iterates the known stage fields in fixed linear order and builds `RunPlan`.

## Key Changes
- Replace `StageCatalog + StageDefinition` with `RunConfig.stages` as the single stage source of truth.
- Make every `StageConfig` expose side-effect-free planning methods:
  - `stage_key`
  - `enabled`
  - `availability(context) -> StageAvailability`
  - `declared_outputs(paths) -> list[ArtifactRef | Path]`
  - `runtime_descriptor() -> StageRuntimeDescriptor`
  - `failure_outcome(error) -> StageOutcome`
- Keep runtime construction out of configs:
  - configs validate and describe
  - `RunConfig.compile_plan()` plans
  - `RuntimeManager` constructs actors/runtimes from descriptors
- Make `RunPlan` compilation a method on `RunConfig`:
  - `RunConfig.compile_plan(path_config, descriptors) -> RunPlan`
  - it walks the fixed `StageBundle` order: `source -> slam -> [ground.align] -> [trajectory.evaluate] -> summary`
  - unavailable enabled stages remain in `RunPlan` with availability reasons, per your chosen preview behavior
- Remove target-doc references implying a central catalog is required:
  - remove `catalog.py`
  - remove `StageCatalog` UML nodes
  - rename “Stage Catalog And Definition” to “RunConfig Stage Bundle”
  - update final UML set from `RunConfig -> StageCatalog -> RunPlan` to `RunConfig -> StageBundle -> RunPlan`

## Interface Shape
```text
RunConfig(BaseConfig, FactoryConfig)
  source / stages.source: SourceStageConfig
  stages.slam: SlamStageConfig
  stages.ground_alignment: GroundAlignmentStageConfig
  stages.trajectory_evaluation: TrajectoryEvaluationStageConfig
  stages.summary: SummaryStageConfig
  compile_plan(...)
  setup_target(...)
...
- Check target doc no longer contains `StageCatalog`.
- Check `RunConfig`, `StageBundle`, and `StageConfig` are consistently described as the planning source of truth.
- Check `setup_target()` remains side-effect free and does not imply actor startup.
- Check `RunPlan` still exists as preview/execution intent with unavailable rows and reasons.

## Assumptions
- Public pipeline remains linear and explicit, not a DAG.
- Stage configs may contain side-effect-free planning methods, but not live runtime startup.
- Runtime construction remains owned by `RuntimeManager`.
 -->
