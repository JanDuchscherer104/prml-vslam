...
| `SlamSessionInit` | removed from `interfaces.slam` | Former streaming SLAM session init DTO. | Replaced by private `SlamStreamingStartInput` behind `SlamStageRuntime`; backend streaming lifecycle receives explicit manifest/reference inputs directly. | `pipeline.stages.slam` | WP-06 | No production compatibility surface remains. | Deleted. | Grep for `SlamSessionInit`; SLAM runtime tests pass. |
...
| `gravity.align` stage key | `pipeline.contracts.stages.StageKey` | Current executable gravity/ground-alignment stage name. | Keep as the target public stage key. | `pipeline.config` / `pipeline.stages.ground_alignment` | WP-02, WP-05, WP-09 | Current runs, manifests, summaries, and stage config sections remain inspectable. | Not deleted. | Stage-key projection and gravity-alignment planning tests. |
...
| `SlamStageConfig` | `pipeline.contracts.request` | Former SLAM request section. | Replaced by stage-local `SlamStageConfig`; persisted backend variants and output policy are stage-owned. | `pipeline.stages.slam.config` | WP-02, WP-06, WP-10 | No request compatibility remains. | Done. | SLAM config parsing and runtime tests. |
...
| `TrajectoryBenchmarkConfig` | `benchmark.contracts` | Former trajectory evaluation policy. | Replaced by `TrajectoryEvaluationPolicy` under `[stages.evaluate_trajectory]`. | `pipeline.stages.trajectory_eval.config` | WP-05, WP-10 | No benchmark-owned trajectory policy remains. | Done. | Trajectory planning/eval tests. |
| `CloudBenchmarkConfig` | `benchmark.contracts` | Former cloud evaluation placeholder policy. | Replaced by the diagnostic `[stages.evaluate_cloud]` config and binding. | `pipeline.stages.cloud_eval.config` | WP-10 | No benchmark-owned cloud policy remains. | Done. | Planning unavailable-row tests. |
...
| `ReferenceReconstructionConfig` | `benchmark.contracts` | Former reference reconstruction policy/config. | Replaced by `[stages.reconstruction]` and stage-owned reconstruction backend variants. | `pipeline.stages.reconstruction.config` | WP-05, WP-10 | No `reference.reconstruct` compatibility remains. | Done. | Reconstruction planning tests. |
...
| `SlamOutputPolicy` | `methods.config_contracts` | Former method-facing output materialization policy. | Stage-owned output policy is canonical for persisted runs; method protocols consume it structurally. | `pipeline.stages.slam.config` | WP-06, WP-10 | No persisted method-owned output policy remains. | Done. | Method and SLAM runtime tests. |
| `SlamBackendConfig` | `methods.config_contracts` | Former base method backend config. | Shrunk to generic runtime options for method protocols/tests only. | `methods.config_contracts` | WP-10 | No discriminated config union remains in methods. | Done. | Backend/runtime tests. |
| `MockSlamBackendConfig` | `pipeline.stages.slam.config` | Stage-owned mock backend config variant. | Methods consume structural `MockSlamBackendOptions` only. | `pipeline.stages.slam.config` / `methods.options` | WP-10 | No method-owned `FactoryConfig` remains. | Done. | Mock backend tests. |
| `VistaSlamBackendConfig` | `pipeline.stages.slam.config` | Stage-owned ViSTA backend config variant. | Methods consume structural `VistaSlamBackendOptions` only. | `pipeline.stages.slam.config` / `methods.options` | WP-10 | No method-owned `FactoryConfig` remains. | Done. | ViSTA config/smoke planning tests. |
| `Mast3rSlamBackendConfig` | `pipeline.stages.slam.config` | Stage-owned MASt3R backend config variant. | Methods consume structural `Mast3rSlamBackendOptions` only. | `pipeline.stages.slam.config` / `methods.options` | WP-10 | No method-owned `FactoryConfig` remains. | Done. | Backend config tests. |
...
| `SourceStageOutput` | `interfaces.ingest` | Source-stage result payload bundling the normalized sequence and optional prepared benchmark inputs. | Keep canonical shared source output DTO for `StageResult.payload`. | `interfaces.ingest` | WP-04 | Downstream source accessors read this before legacy split fields. | Not deleted. | Source runtime and result-store tests. |
...
| `MethodId` | `methods.config_contracts` | Stage-facing discriminator moved to `pipeline.stages.slam.config`; method-local id remains internal to wrapper protocols. | pipeline stages / methods | Backend config tests. |
