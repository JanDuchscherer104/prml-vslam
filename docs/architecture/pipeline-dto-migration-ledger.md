...
| `gravity.align` stage key | `pipeline.contracts.stages.StageKey` | Historical executable gravity/ground-alignment stage name. | Normalize to target `align.gravity` during artifact inspection only; current runtime emits `align.gravity`. | `pipeline.artifact_inspection` / `alignment.stage` | WP-02, WP-05, WP-09 | Old runs, manifests, and summaries remain inspectable without allowing old keys in new runtime plans. | Live enum alias deleted after old-run inspection coverage. | Artifact-inspection historical-key test plus gravity-alignment planning tests. |
| `trajectory.evaluate` / `evaluate.trajectory` stage keys | `pipeline.contracts.stages.StageKey` | Historical trajectory-evaluation stage names. | Normalize old persisted keys to target `eval.trajectory` during artifact inspection only; current runtime emits `eval.trajectory`. | `pipeline.artifact_inspection` / `eval.stage_trajectory` | WP-02, WP-05, WP-09, WP-10 | Old runs, manifests, summaries, and current stage registry remain inspectable. | Live enum aliases deleted after old-run inspection coverage. | Artifact-inspection historical-key test plus trajectory-evaluation planning tests. |
...
| `cloud.evaluate` / `evaluate.cloud` stage keys | `pipeline.contracts.stages.StageKey` | Historical placeholder dense-cloud/point-set evaluation key. | Normalize old persisted keys to target `eval.points` during artifact inspection only; current runtime emits `eval.points`. | `pipeline.artifact_inspection` / future eval stage owner | WP-02, WP-09, WP-10 | Old runs, manifests, summaries, and current stage registry remain inspectable. | Live enum aliases deleted after old-run inspection coverage. | Artifact-inspection historical-key test plus unavailable-placeholder planning tests. |
