...
incremental scene updates and allowing an evaluation of the method's real-time performances.

Any run complied from a given `RunConfig` into a deterministic sequence of stages, consisting of normalized sources, vSLAM, optional gravity, Sim(3) and ICP
...
Streaming preserves the same stage order but moves the runtime boundary to a bounded actor pipeline, where all stages are executed concurrently and can allocate resources as configured by the `RunConfig`.
...
These conditions ensure that the run-time performance of the method can be evaluated independently of computational overhead of the other stages and the pipeline framework itself.
