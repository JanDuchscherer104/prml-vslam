...
when intrinsics, metric scale, and the reference coordinate system are not uniformly available. A
rendered overlay is insufficient evidence: the source frame, pose convention, sampling policy,
depth units, trajectory alignment, and point-cloud registration determine the measured quantity.
...
#figure(
  table(
    ...
    [Supports debugging while keeping rendered views separate from metric records.],
    bottomrule(),
  ),
...
LingBot-Map, Rerun visualization, trajectory evaluation, and ICP-based cloud placement. Dense-cloud
scores and efficiency numbers remain validation targets: they are admissible only after the same
method--dataset matrix, sampling policy, and artifact checks are frozen for all compared methods.
