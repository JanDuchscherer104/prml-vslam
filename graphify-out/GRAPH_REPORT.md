...
## Corpus Check
- 272 files · ~1,067,793 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4116 nodes · 19154 edges · 33 communities detected
- Extraction: 31% EXTRACTED · 69% INFERRED · 0% AMBIGUOUS · INFERRED: 13181 edges (avg confidence: 0.59)
...
2. `SequenceManifest` - 319 edges
3. `ArtifactRef` - 282 edges
4. `MethodId` - 264 edges
...
Cohesion: 0.02
Nodes (329): _build_artifacts(), _DensePredictionArtifacts, _ensure_uint8_rgb_from_uimg(), _estimate_camera_intrinsics_from_frame(), _InProcessManager, _InProcessValue, LingbotMapSlamBackend, _LingbotRuntime (+321 more)
...
Cohesion: 0.01
Nodes (338): _preprocess_images_with_lingbot(), build_advio_comparison_trajectories(), build_crowd_density_figure(), build_local_readiness_figure(), build_scene_attribute_figure(), build_scene_mix_figure(), handle_advio_preview_action(), load_advio_explorer_sample() (+330 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (241): artifact_ref(), Build one stable artifact reference for a materialized path., BaseStageRuntime, FailureFingerprint, Reject negative custom resource quantities., Allow only exact artifact keys or safe ``prefix:*`` selectors., Return the declared output paths for a generic stage section., Return whether the configured stage can run. (+233 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (294): resolve(), BaseConfig, _ConfigFactory, from_toml(), _normalize_value(), Shared config and config-as-factory helpers for the repository.  This module own, Render the config as a Rich tree for quick human inspection., Augment :class:`BaseData` with deterministic TOML IO and config inspection. (+286 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (309): MethodId, AppContext, CameraIntrinsics, Describe one camera raster in a backend- and dataset-neutral way.      Use this, AdvioSourceConfig, CloudAlignmentStageConfig, CloudEvaluationStageConfig, CloudMetricId (+301 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (226): GroundAlignmentMetadata, Result of one derived ground-plane alignment attempt.      When :attr:`applied`, _apply_snapshot_fallbacks(), _candidate_from_root(), _canonical_path_rows(), _derive_slam_artifacts(), discover_run_artifact_roots(), _file_inventory() (+218 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (198): _as_numpy(), _build_lingbot_artifacts(), _decode_pose_predictions(), _expect_lingbot_config(), _extract_dense_prediction_artifacts(), _flatten_depth_points(), _images_chw_to_rgb(), _is_cuda_oom() (+190 more)
...
Cohesion: 0.02
Nodes (187): GroundPlaneModel, GroundPlaneVisualizationHint, Alignment result DTOs shared outside the alignment package.  These datamodels de, Dominant ground-plane hypothesis expressed in native ``world`` coordinates., Finite plane-patch geometry ready for visualization consumers., Render the shared intrinsics matrix in the compact LaTeX form used by UI surface, GroundAlignmentConfig, Inputs required to derive ground-alignment metadata from SLAM outputs. (+179 more)
...
Cohesion: 0.04
Nodes (91): _ape_error_colors(), attach_recording_sinks(), augment_viewer_recording_with_ground_plane(), build_default_blueprint(), create_recording_stream(), _decimate_rows(), _entity_token(), evaluation_metric_root() (+83 more)

### Community 12 - "Community 12"
Cohesion: 0.05
Nodes (75): FactoryConfig, Mixin for configs that construct one runtime owner or adapter.      This pattern, Return the runtime type or owner constructed by :meth:`setup_target`., Instantiate or build the runtime object described by this config., Normalize nested config values into JSON-friendly primitives., Load the validated config from TOML text, bytes, or a file path., caller_namespace(), configure_logging() (+67 more)
...
Cohesion: 0.09
Nodes (34): DenseCloudSelectionConfig, Open3dTsdfBackendConfig, Provide the package-local runtime contract shared by reconstruction configs., Return the user-facing reconstruction label., Configure the minimal Open3D TSDF reconstruction backend.      The repo targets, Return the concrete reconstruction backend type., Instantiate the Open3D TSDF backend while ignoring unrelated kwargs., ReconstructionBackendConfig (+26 more)
...
- **Why does `Test package helpers and suites for PRML VSLAM.` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 12`, `Community 13`, `Community 14`?**
  _High betweenness centrality (0.152) - this node is a cross-community bridge._
- **Why does `StageKey` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 11`, `Community 14`, `Community 16`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `SequenceManifest` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`?**
...
- **Are the 316 inferred relationships involving `SequenceManifest` (e.g. with `OfflineSlamBackend` and `StreamingSlamBackend`) actually correct?**
  _`SequenceManifest` has 316 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 261 inferred relationships involving `MethodId` (e.g. with `RunConfigOverrideCommand` and `_RerunViewerProcess`) actually correct?**
  _`MethodId` has 261 INFERRED edges - model-reasoned connections that need verification._