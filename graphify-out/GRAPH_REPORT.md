...
## Corpus Check
- 263 files · ~606,032 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4175 nodes · 22508 edges · 43 communities detected
- Extraction: 25% EXTRACTED · 75% INFERRED · 0% AMBIGUOUS · INFERRED: 16982 edges (avg confidence: 0.56)
...
1. `StageKey` - 615 edges
2. `MethodId` - 378 edges
3. `RunConfig` - 369 edges
4. `PathConfig` - 323 edges
5. `SequenceManifest` - 310 edges
6. `ArtifactRef` - 308 edges
7. `RunSnapshot` - 301 edges
8. `ReferenceSource` - 291 edges
9. `StageRuntimeStatus` - 268 edges
10. `StageRuntimeUpdate` - 257 edges
...
Cohesion: 0.02
Nodes (263): AdvioDownloadManager, _ensure_directory_parent(), Return the cache directory used for downloaded scene archives., Return one catalog scene by id., Return local availability status for every catalog scene., Download selected ADVIO scenes and extract the requested modalities., archive_member_matches(), list_local_sequence_ids() (+255 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (340): transform_trajectory_with_alignment(), GroundAlignmentMetadata, GroundPlaneModel, GroundPlaneVisualizationHint, Alignment result DTOs shared outside the alignment package.  These datamodels de, Dominant ground-plane hypothesis expressed in native ``world`` coordinates., Finite plane-patch geometry ready for visualization consumers., Result of one derived ground-plane alignment attempt.      When :attr:`applied` (+332 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (270): InputArtifactDiagnostics, Inspection helpers for persisted pipeline run artifact roots., One submitted run attempt found in a persisted event log., Structured inspection result for one persisted pipeline run., Discover method-level run roots under the configured artifact directory., Load typed metadata and path inventory for one persisted run root., One selectable persisted method-level run artifact root., Shallow diagnostics for materialized offline input artifacts. (+262 more)

### Community 3 - "Community 3"
Cohesion: 0.05
Nodes (347): MethodId, PipelineBackend, Execute, monitor, and tear down pipeline runs.      Implementations own the conc, Ray-backed backend for plan execution and run attachment.  This module owns subs, Forward a stop request to the named coordinator actor., Forward a stop request to the named coordinator actor., Fetch the latest projected snapshot from the coordinator actor., Fetch the latest projected snapshot from the coordinator actor. (+339 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (286): Convert an ADVIO pose CSV into a TUM trajectory file., write_advio_pose_tum(), resolve(), _apply_snapshot_fallbacks(), _candidate_from_root(), _canonical_path_rows(), _derive_slam_artifacts(), discover_run_artifact_roots() (+278 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (310): advio_basis_metadata(), advio_basis_provenance(), AdvioBasisMetadata, AdvioRawCoordinateBasis, basis_for_pose_source(), _flatten_matrix(), _pose_matrix(), ADVIO coordinate-basis normalization helpers.  ADVIO stores Apple-family traject (+302 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (237): artifact_ref(), artifact_visualizations(), Build one stable artifact reference for a materialized path., Return neutral visualization items for completed durable artifacts., Log a warning message., ReconstructionInputSourceKind, ReconstructionStageInput, SlamOfflineStageInput (+229 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (222): Controller helpers for the ADVIO Streamlit page., Persist the current ADVIO download-form state., Persist the current explorer selection and load its offline sample., Keep persisted preview state aligned with the runtime snapshot., Apply one preview-form action and return an error message when it fails., build_context(), _build_pages(), Bootstrap helpers for the packaged PRML VSLAM Streamlit app. (+214 more)

### Community 8 - "Community 8"
Cohesion: 0.01
Nodes (284): build_advio_comparison_trajectories(), build_crowd_density_figure(), build_local_readiness_figure(), build_scene_attribute_figure(), build_scene_mix_figure(), build_advio_page_data(), handle_advio_preview_action(), load_advio_explorer_sample() (+276 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (170): Canonical ViSTA-SLAM backend adapter (offline + streaming)., ViSTA-SLAM backend implementing offline and streaming contracts., Load upstream OnlineSLAM and retain backend-owned streaming state., Consume one streaming frame through the active ViSTA runtime., Retrieve pending ViSTA live updates without exposing runtime state., Finalize the active ViSTA streaming runtime and clear it., Run ViSTA-SLAM over normalized offline observations and persist artifacts., VistaSlamBackend (+162 more)

### Community 10 - "Community 10"
Cohesion: 0.03
Nodes (113): get_events(), get_snapshot(), Backend boundary between launch surfaces and execution substrates.  This module, _coordinator_actor_options(), RayPipelineBackend, stop_run(), submit_run(), _enter_page() (+105 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (132): Render directly via Rich for structured or non-log output., ArxivSourceSpec, download_file(), fetch_pdf(), fetch_tex_source(), from_json(), load_manifest(), main() (+124 more)
...
Cohesion: 0.32
Nodes (2): _resolve_reference(), TrajectoryAlignmentRuntime
...
- **Thin community `Community 19`** (8 nodes): `runtime.py`, `_resolve_reference()`, `TrajectoryAlignmentRuntime`, `.__init__()`, `._run()`, `.run_offline()`, `.status()`, `.stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (4 nodes): `_valid_artifact_selector()`, `validate_artifact_key_selectors()`, `validate_custom_resources()`, `config.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `Test package helpers and suites for PRML VSLAM.` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 12`, `Community 13`, `Community 15`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `StageKey` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 9`, `Community 10`, `Community 14`, `Community 19`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `FrameTransform` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 5`, `Community 6`, `Community 7`, `Community 9`, `Community 10`, `Community 13`, `Community 18`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Are the 612 inferred relationships involving `StageKey` (e.g. with `RunConfigOverrideCommand` and `_RerunViewerProcess`) actually correct?**
  _`StageKey` has 612 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 364 inferred relationships involving `RunConfig` (e.g. with `RunConfigOverrideCommand` and `_RerunViewerProcess`) actually correct?**
  _`RunConfig` has 364 INFERRED edges - model-reasoned connections that need verification._
