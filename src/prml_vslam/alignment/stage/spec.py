...
def _failure_fingerprint(context: PipelineExecutionContext) -> FailureFingerprint:
    ...
    return FailureFingerprint(
        config_payload=context.run_config.stages.align_ground.ground,
        input_payload={
            "trajectory_tum": slam.trajectory_tum.fingerprint,
            "dense_points_ply": None if slam.dense_points_ply is None else slam.dense_points_ply.fingerprint,
            "sparse_points_ply": None if slam.sparse_points_ply is None else slam.sparse_points_ply.fingerprint,
        },
    )
