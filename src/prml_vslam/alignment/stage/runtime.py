...
class GroundAlignmentRuntime(
    ...
    def submit_stream_item(self, item: GroundAlignmentKeyframeSample) -> None:
        """Accept one SLAM keyframe pointmap and run interval-gated RANSAC."""
        ...
        interval = self._streaming_input.config.ransac_interval_keyframes
        progress_message = (
            f"Accepted {self._accepted_keyframe_count} keyframes; waiting for RANSAC interval {interval}."
        )
        metadata_update: GroundAlignmentMetadata | None = None
        if self._accepted_keyframe_count % interval == 0:
            metadata = self._estimate_streaming_metadata()
            self._latest_metadata = metadata
            metadata_update = metadata
            progress_message = (
                f"Updated streaming ground alignment from {self._accepted_keyframe_count} keyframes."
                if metadata.applied
                else f"Skipped streaming ground alignment update: {metadata.skip_reason or 'no reliable plane'}"
            )
        self._status = self._status.model_copy(
            update={
                "lifecycle_state": StageStatus.RUNNING,
        ...
        if metadata_update is not None:
            self._pending_updates.append(
                StageRuntimeUpdate(
                    ...
                    semantic_events=[metadata_update],
    ...
    def finish_streaming(self) -> StageResult:
        ...
        if metadata is None:
            metadata = GroundAlignmentMetadata(
                ...
                skip_reason="No streaming ground-alignment estimate was produced before finalization.",
            )
