...
class VistaSlamConfig(BaseConfig):
    """Algorithm-level hyperparameters forwarded to OnlineSLAM."""
    ...
    stride: int = 25
    """Keyframe stride used as a fallback when optical-flow detection is active."""

    flow_thres: float = 5.0
    """Optical-flow magnitude threshold that triggers a new keyframe."""

    keyframe_detection: str = "flow_stride"
    """Keyframe-selection strategy: 'stride' or 'flow_stride'."""
...
class VistaSlamBackendConfig(BaseConfig):
    ...
    """Path to the cloned vista-slam repository (submodule root)."""
    ...
    """Path to the ORB vocabulary file used by the loop detector."""

    slam: VistaSlamConfig = Field(default_factory=VistaSlamConfig)
    """Algorithm-level hyperparameters for OnlineSLAM."""
