...
| `window_size` / `overlap_keyframes` | windowed inference controls | `128` / `8` | Accepted windowed profile from the full TUM cabinet run. |
...
| `confidence_threshold` | upstream viewer/export default varies by entry point | `0.5` | Filters low-confidence depth before durable point-cloud export while retaining cabinet geometry for alignment. |
