...
| `image_size` | `518` width, about `518x378` in the paper | `518` | Keeps the checkpoint-native patch grid; the adapter does not rewrite positional embeddings for smaller rasters. |
...
| `use_amp` | bfloat16/float16 inference through CUDA autocast | `use_amp=true` | Matches upstream precision handling while reducing memory pressure. |
...
On constrained GPUs, keep `image_size=518` unless upstream provides a
checkpoint-compatible loading path for another patch grid. The main config uses
`keyframe_interval=6` as the current memory-fit candidate. If FlashInfer is
installed or a larger GPU is available, retry `keyframe_interval = "auto"` and
`use_sdpa = false`.
