...
- The package owns method IDs, backend config, output policy, runtime updates,
  the canonical ViSTA backend integration, and the canonical MASt3R backend
  integration.
...
- `methods.mast3r` owns MASt3R-SLAM runtime adaptation, prerequisite checks,
  live pointmap normalization, and dense-only artifact normalization.
