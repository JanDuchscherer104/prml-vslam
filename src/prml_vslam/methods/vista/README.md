...
- Offline frames enter through `VistaSlamBackend.run_sequence()`, while streaming frames enter through `VistaSlamRuntime.step()`. Both paths pass repository RGB arrays into `UpstreamVistaFramePreprocessor.prepare()`.
