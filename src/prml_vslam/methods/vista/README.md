...
- Offline frames are dematerialized by the SLAM stage/source helpers and enter through `VistaSlamBackend.run_observations()`, while streaming frames enter through `VistaSlamRuntime.step()`. Both paths pass repository RGB arrays into `UpstreamVistaFramePreprocessor.prepare()`.
