...
from prml_vslam.utils.geometry import pointmap_from_depth, write_point_cloud_ply, write_tum_trajectory
...
class VistaSlamSession:
    ...
    def step(self, frame: FramePacket) -> SlamUpdate:
        ...
        update = self._build_live_update(seq=frame.seq, timestamp_ns=frame.timestamp_ns, view_index=self._frame_count)
        self._frame_count += 1
        return update
    ...
    def _build_live_update(
        ...
        *,
        ...
        pointmap = _build_live_pointmap(view)
...
def _build_live_pointmap(view: object) -> np.ndarray | None:
    """Convert one upstream view payload into a camera-frame pointmap."""
    depth_map = np.asarray(getattr(view, "depth", None), dtype=np.float32)
    intrinsics_matrix = np.asarray(getattr(view, "intri", None), dtype=np.float64)
    if depth_map.ndim != 2 or intrinsics_matrix.shape != (3, 3):
        return None
    ...
    return pointmap_from_depth(depth_map, intrinsics)
