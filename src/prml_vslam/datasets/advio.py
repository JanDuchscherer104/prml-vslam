...
_GYROSCOPE_FILE_NAMES = ("gyroscope.csv", "gyro.csv")
_PLATFORM_LOCATION_FILE_NAMES = ("platform-location.csv", "platform-locations.csv")
...
_TIMED_MODALITY_SPECS = (
    ...
    ("iphone_gyro", "iPhone gyroscope", "imu", Path("iphone/gyroscope.csv"), "Angular velocity"),
    (
        "iphone_magnetometer",
        "iPhone magnetometer",
        "imu",
    ...
    (
        ...
        "baseline",
        Path("iphone/platform-location.csv"),
...
class AdvioSequenceConfig(BaseConfig):
    ...
    def sequence_dir(self) -> Path:
        ...
        try:
            return _resolve_sequence_dir(self)
    ...
    def ground_truth_path(self) -> Path:
        ...
        return _resolve_ground_truth_path(self.sequence_dir)
...
class AdvioSequencePaths(BaseConfig):
    ...
    def resolve(cls, config: AdvioSequenceConfig) -> Self:
        ...
        ground_truth_path = _resolve_ground_truth_path(sequence_dir)
        resolved = cls(
            config=config,
...
class AdvioSequence(BaseConfig):
    ...
    def load_iphone_imu(self) -> list[AdvioImuSample]:
        ...
        return load_advio_imu_samples(
            ...
            gyroscope_path=_resolve_first_existing_named_path(sequence_dir / "iphone", _GYROSCOPE_FILE_NAMES),
        )
...
def download_advio_sequence(
    ...
    *,
    ...
    if force:
        for candidate in _iter_sequence_dir_candidates(config):
            if candidate.exists():
...
def summarize_advio_sequence(config: AdvioSequenceConfig) -> AdvioSequenceSummary:
    ...
    for slug, label, family, relative_path, detail in _TIMED_MODALITY_SPECS:
        try:
            path = _resolve_advio_relative_path(sequence_dir, relative_path)
        except FileNotFoundError:
            continue
        if not path.exists():
            continue
...
def _resolve_sequence_dir(config: AdvioSequenceConfig) -> Path:
    ...
    for candidate in _iter_sequence_dir_candidates(config):
        if not candidate.is_dir():
            continue
        if _sequence_dir_has_required_files(candidate):
            return candidate
...
def _iter_sequence_dir_candidates(config: AdvioSequenceConfig) -> tuple[Path, Path]:
    """Return the known extracted-directory candidates for one ADVIO sequence."""
    return (
...
def _sequence_dir_has_required_files(sequence_dir: Path) -> bool:
    """Return whether ``sequence_dir`` contains the required ADVIO core files."""
    required_paths = (
        sequence_dir / "iphone" / "frames.mov",
        sequence_dir / "iphone" / "frames.csv",
        sequence_dir / "pixel" / "arcore.csv",
    )
    if not all(path.exists() for path in required_paths):
        return False
    try:
        _resolve_ground_truth_path(sequence_dir)
    except FileNotFoundError:
        return False
    return True


def _resolve_advio_relative_path(sequence_dir: Path, relative_path: Path) -> Path:
    """Resolve one ADVIO relative path, including upstream-vs-legacy aliases."""
    match relative_path.as_posix():
        case "ground-truth/poses.csv":
            return _resolve_ground_truth_path(sequence_dir)
        case "iphone/gyroscope.csv":
            return _resolve_first_existing_named_path(sequence_dir / "iphone", _GYROSCOPE_FILE_NAMES)
        case "iphone/platform-location.csv":
            return _resolve_first_existing_named_path(sequence_dir / "iphone", _PLATFORM_LOCATION_FILE_NAMES)
        case _:
            return sequence_dir / relative_path


def _resolve_ground_truth_path(sequence_dir: Path) -> Path:
    """Resolve the ground-truth pose CSV under ``sequence_dir``."""
    return _resolve_first_existing_named_path(sequence_dir / "ground-truth", _POSE_FILE_NAMES)


def _resolve_first_existing_named_path(root: Path, names: tuple[str, ...]) -> Path:
    """Resolve the first existing named child path under ``root``."""
    return resolve_first_existing(root, names)
