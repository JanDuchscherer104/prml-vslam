...
Today `prml_vslam.pipeline` is primarily a typed planning and bounded-runtime
package. [`contracts.py`](./contracts.py) defines the public contracts such as
...
[`RunPlan`](./contracts.py); [`workspace.py`](./workspace.py) defines the
capture-manifest helpers used when inputs are materialized; [`demo.py`](./demo.py)
owns shared request-template helpers; and the actual bounded runtime is owned
by [`RunService`](./run_service.py) and
[`PipelineSessionService`](./session.py).

That gives the repository a real executable slice today, but not yet the full
backend described in [`REQUIREMENTS.md`](./REQUIREMENTS.md). The current launch
surfaces are the Streamlit [`Pipeline` page](../app/pages/pipeline.py) and the
CLI [`pipeline-demo`](../main.py) command. Both configure or load a
[`RunRequest`](./contracts.py), resolve a source, and hand the work into the
pipeline-owned runtime. The currently executable stage subset is limited to
`ingest`, `slam`, and `summary`; the planner can already describe reference and
evaluation stages, but the bounded runtime rejects them until explicit runtime
support is added.

The target backend is still slightly larger and slightly simpler than the
current implementation. It should converge on a real `OfflineRunner` and a real
`StreamingRunner`, still fronted by [`RunService`](./run_service.py), with
source resolution owned by the source seams, method execution owned by thin
wrappers around upstream systems, evaluation owned by
[`prml_vslam.eval`](../eval/README.md), and the app kept as a thin launch and
monitoring surface. The repo therefore already has the right typed seams, but
the reusable runner layer and the real external backend wrappers are still the
main missing implementation steps.

If you want to locate the current bounded demo in code, the shortest path is:
[`../app/pages/pipeline.py`](../app/pages/pipeline.py) or
[`../main.py`](../main.py) for launch, [`demo.py`](./demo.py) for persisted
request helpers, [`run_service.py`](./run_service.py) for runtime entry,
[`session.py`](./session.py) for session lifetime and finalization,
[`../datasets/advio_service.py`](../datasets/advio/advio_service.py) plus
[`../datasets/advio_sequence.py`](../datasets/advio/advio_sequence.py) for the
ADVIO source path, [`../io/cv2_producer.py`](../io/cv2_producer.py) for the
generic replay stream, and [`../methods/mock_vslam.py`](../methods/mock_vslam.py)
for the repository-local mock backend.
...
The ownership model in this package follows the rules documented in
[`docs/architecture/interfaces-and-contracts.md`](../../../docs/architecture/interfaces-and-contracts.md).
Repo-wide shared data models such as
[`CameraIntrinsics`](../interfaces/camera.py),
[`SE3Pose`](../interfaces/camera.py), and
[`FramePacket`](../interfaces/runtime.py) belong in
[`prml_vslam.interfaces`](../interfaces/runtime.py). Repo-wide shared behavior
seams such as [`FramePacketStream`](../protocols/runtime.py),
[`OfflineSequenceSource`](../protocols/source.py), and
[`StreamingSequenceSource`](../protocols/source.py) belong in
[`prml_vslam.protocols`](../protocols/source.py). The pipeline package owns its
own DTOs and manifests in [`contracts.py`](./contracts.py), which is why
[`RunRequest`](./contracts.py), [`RunPlan`](./contracts.py),
[`SequenceManifest`](./contracts.py), [`SlamArtifacts`](./contracts.py),
[`StageManifest`](./contracts.py), and [`RunSummary`](./contracts.py) live
here rather than in shared namespaces.

The flip side is just as important: the pipeline does not own every symbol that
it consumes. SLAM behavior seams such as
[`OfflineSlamBackend`](../methods/protocols.py),
[`StreamingSlamBackend`](../methods/protocols.py),
[`SlamBackend`](../methods/protocols.py), and
[`SlamSession`](../methods/protocols.py) belong to
[`prml_vslam.methods.protocols`](../methods/protocols.py), not to the pipeline
package. Streamlit-only UI state such as
[`PipelinePageState`](../app/models.py) belongs to
[`prml_vslam.app.models`](../app/models.py), not to pipeline contracts. This
separation is what keeps one semantic concept attached to one owning module
instead of letting app state, runtime behavior, and benchmark contracts drift
into parallel shapes.

The practical benefit for the team is that the public center of gravity stays
small. A teammate can treat
[`RunRequest`](./contracts.py), [`RunPlan`](./contracts.py),
[`SequenceManifest`](./contracts.py), [`SlamArtifacts`](./contracts.py),
[`RunSummary`](./contracts.py), [`SlamBackend`](../methods/protocols.py), and
[`SlamSession`](../methods/protocols.py) as the important backend-facing
surfaces, while package-local helper types remain implementation details unless
...
- a live camera feed
- a device stream such as Record3D USB
...
Planning exists to convert a high-level, config-shaped
[`RunRequest`](./contracts.py) into a deterministic execution contract before
any expensive runtime work starts. In practice,
[`RunPlannerService.build_run_plan(...)`](./services.py) validates request
compatibility, selects enabled stages, assigns stable
[`RunPlanStageId`](./contracts.py) values, resolves canonical artifact paths
through [`PathConfig.plan_run_paths(...)`](../utils/path_config.py), and gives
the app or CLI a previewable [`RunPlan`](./contracts.py) before any source is
opened or any backend session is started.

The diagram below summarizes that planning phase. The Mermaid source lives in
[`docs/figures/mermaid/mermaid_pipeline_planning_phase.mmd`](../../../docs/figures/mermaid/mermaid_pipeline_planning_phase.mmd).

![Pipeline planning phase](../../../docs/figures/mermaid/mermaid_pipeline_planning_phase.svg)

### What Manifests And Artifacts Are

This repository is manifest-driven and artifact-first. The two words are
related, but they do not mean the same thing.

A **manifest** is a typed description of normalized data or executed work. It
exists to tell later stages what is available, where it lives, and how it
should be interpreted. A manifest is therefore about structure, provenance, and
stable references rather than about carrying the heavy payload itself.

An **artifact** is the actual durable payload that a stage produces or reuses:
for example a trajectory file, a point cloud, or a persisted log. Artifacts
are the concrete outputs that teammates inspect, compare, evaluate, and pass to
downstream tools.

The current implemented manifests are:

- [`CaptureManifest`](./workspace.py)
  - describes a materialized image sequence prepared from a source capture
- [`SequenceManifest`](./contracts.py)
  - describes the normalized sequence boundary consumed by the main benchmark stages
- [`StageManifest`](./contracts.py)
  - describes one executed stage through fingerprints, outputs, and status
- [`RunSummary`](./contracts.py)
  - describes the final run-level outcome and stage-status map

The current implemented artifact contracts are:

- [`ArtifactRef`](./contracts.py)
  - a typed handle to one durable artifact path plus its kind and fingerprint
- [`SlamArtifacts`](./contracts.py)
  - the concrete output bundle produced by the SLAM stage, including the normalized
    trajectory and optional geometry or preview artifacts

In practice, that means files such as `input/sequence_manifest.json`,
`summary/stage_manifests.json`, and `summary/run_summary.json` are manifest
artifacts describing normalized inputs or executed stages, while files such as
`slam/trajectory.tum`, `slam/sparse_points.ply`, `dense/dense_points.ply`, and
live preview logs are payload artifacts produced by execution.

The next diagram summarizes the durable data boundaries that matter during one
run. The Mermaid source lives in
[`docs/figures/mermaid/mermaid_pipeline_boundaries.mmd`](../../../docs/figures/mermaid/mermaid_pipeline_boundaries.mmd).

![Pipeline artifact and runtime boundaries](../../../docs/figures/mermaid/mermaid_pipeline_boundaries.svg)
...
The first diagram below is the high-level lifecycle view. It shows how one
persisted
...
[`docs/figures/mermaid/mermaid_pipeline_request_flow.mmd`](../../../docs/figures/mermaid/mermaid_pipeline_request_flow.mmd).

![Persisted request lifecycle](../../../docs/figures/mermaid/mermaid_pipeline_request_flow.svg)

If you want the exact call-order view instead of the structural phase view, the
sequence diagram below keeps the original interaction-oriented perspective. Its
Mermaid source lives in
[`docs/figures/mermaid/mermaid_pipeline_request_sequence.mmd`](../../../docs/figures/mermaid/mermaid_pipeline_request_sequence.mmd).

![Persisted request sequence](../../../docs/figures/mermaid/mermaid_pipeline_request_sequence.svg)
...
[`docs/figures/mermaid/mermaid_pipeline_session_state.mmd`](../../../docs/figures/mermaid/mermaid_pipeline_session_state.mmd).

![Pipeline session state lifecycle](../../../docs/figures/mermaid/mermaid_pipeline_session_state.svg)
...
See [Supported Stage Types](#supported-stage-types). The short answer is that
the planner can already describe more stages than the current bounded runtime
can execute, but the executable slice today is limited to `ingest`, `slam`, and
`summary`.

### Which Modules Own Which Boundaries?

See [Ownership Model](#ownership-model). The main distinction is that
[`contracts.py`](./contracts.py) owns pipeline DTOs and manifests,
[`services.py`](./services.py) owns planning, [`run_service.py`](./run_service.py)
and [`session.py`](./session.py) own the bounded runtime, while shared source
and SLAM behavior seams stay in [`protocols`](../protocols/source.py) and
...
See [What Manifests And Artifacts Are](#what-manifests-and-artifacts-are),
[Supported Stage Types](#supported-stage-types), and
[Artifact Layout](#artifact-layout). The short answer is that
`sequence_manifest.json`, `stage_manifests.json`, and `run_summary.json` are
the always-persisted manifest side of the current runnable slice, while
`trajectory.tum` is the always-persisted SLAM artifact and the point-cloud or
preview-log outputs remain optional.
