...
from prml_vslam.utils import iter_video_frames
...
class SessionManager:
    ...
    def create_session(
        ...
        *,
        ...
        if resolved_mode is PipelineMode.BATCH:
            if video_path is None:
                msg = "video_path is required for batch mode"
                raise ValueError(msg)
        ...
        if resolved_mode is PipelineMode.STREAMING:
            self._ensure_stream_capture_paths(sess)
            self._write_stream_capture_manifest(sess)
        self._sessions[sid] = sess
        logger.info("Created session %s (mode=%s, method=%s)", sid, resolved_mode.value, method.value)
        return sess
    ...
    def iterate_offline(self, session_id: str) -> Iterator[tuple[str, list[Envelope], int]]:
        ...
        for action, _result, state in sess.burr_app.iterate(halt_after=["export"]):
            ...
            step_outputs: list[Envelope] = list(state.get("step_outputs", [])) if action_name == "slam" else []
            if step_outputs:
                sess._outputs.extend(step_outputs)
            yield action_name, step_outputs, int(state.get("frame_index", -1))
    ...
    def close_session(self, session_id: str) -> list[Envelope]:
        ...
        if sess.mode is PipelineMode.STREAMING:
            self._write_stream_capture_manifest(sess)
    ...
    def _resolve_mode(mode: PipelineMode | str) -> PipelineMode:
        try:
            return mode if isinstance(mode, PipelineMode) else PipelineMode(mode)
        except ValueError as exc:
            msg = f"Unsupported pipeline mode: {mode!r}"
