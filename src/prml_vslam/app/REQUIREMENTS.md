...
- The `Pipeline` page is a run console over supported `RunConfig` templates, app-executable source/backend/stage options, generated `RunPlan` previews, and offline plus bounded streaming execution.
...
- The app does not own transport decoding, dataset normalization, pipeline
  semantics, backend orchestration, stage-policy logic, or viewer artifact
  semantics.
