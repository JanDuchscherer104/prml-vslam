...
class CloudEvaluationRuntime(OfflineStageRuntime[CloudEvaluationStageInput]):
    """Compute dense-cloud metrics over already aligned point clouds."""
    ...
    def _run(self, input_payload: CloudEvaluationStageInput) -> StageResult:
        artifact = DenseCloudEvaluationService().compute_dense_evaluation(selection=input_payload.selection)
        artifacts = _artifact_map(artifact)
        metrics: dict[str, float | int | str] = dict(artifact.metrics)
        outcome = StageOutcome(
            stage_key=StageKey.CLOUD_EVALUATION,
            status=StageStatus.COMPLETED,
            config_hash=stable_hash({"f_score_threshold_m": input_payload.selection.f_score_threshold_m}),
            input_fingerprint=stable_hash(
                {
                    "reference_cloud_path": input_payload.selection.reference_cloud_path,
                    "estimate_cloud_path": input_payload.selection.estimate_cloud_path,
                }
            ),
            artifacts=artifacts,
            metrics=metrics,
