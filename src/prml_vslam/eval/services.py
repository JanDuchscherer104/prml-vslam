...
class TrajectoryEvaluationService(TrajectoryEvaluator):
    ...
    def discover_runs(self, sequence_slug: str | None) -> list[DiscoveredRun]:
        ...
        outputs plus the persisted source ``sequence_manifest.json`` when the
        run directory name does not include the sequence slug. It does not
        validate metric availability or compute missing results.
        """
        ...
        for trajectory_path in sorted(self.path_config.artifacts_dir.glob("**/slam/trajectory.tum")):
            ...
            if not _run_matches_sequence(run_root, relative_parts=relative_parts, sequence_slug=sequence_slug):
                continue
            method = next(
            ...
            runs.append(
                DiscoveredRun(
                    ...
                    point_cloud_path=_optional_point_cloud_path(run_root),
                    method=method,
...
def _optional_point_cloud_path(run_root: Path) -> Path | None:
    for relative_path in (Path("slam/dense_points.ply"), Path("slam/point_cloud.ply")):
...
def _run_matches_sequence(run_root: Path, *, relative_parts: tuple[str, ...], sequence_slug: str) -> bool:
    ...
    if not manifest_path.exists():
        return False
    try:
        manifest = SequenceManifest.model_validate_json(manifest_path.read_text(encoding="utf-8"))
    except ValueError:
        return False
    return manifest.sequence_id == sequence_slug
...
def _read_non_empty_point_cloud(path: Path, *, label: str, operation: str = "alignment") -> Any:
    if not path.exists():
