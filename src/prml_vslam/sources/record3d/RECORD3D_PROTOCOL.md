...
| --- | --- | --- |
| RGB | yes | `Observation.rgb` |
| Depth | yes | `Observation.depth_m` |
| Confidence | yes | `Observation.confidence` |
| Intrinsics | yes | `Observation.intrinsics` |
| Camera pose | yes | `Observation.T_world_camera` |
| Device type | yes | `Observation.provenance.device_type` |
...
| --- | --- | --- |
| RGB | yes, in the right half of the composite frame | `Observation.rgb` |
| Depth | yes, HSV-encoded in the left half of the composite frame | decoded but omitted from `Observation` until Wi-Fi can provide a real pose |
| Intrinsics | yes, via metadata key `K` | `Observation.intrinsics` when parsing succeeds |
...
| --- | --- | --- | --- |
| USB | [record3d.py](./record3d.py) | `rgb`, `depth_m`, `confidence`, `intrinsics`, `T_world_camera`, provenance device fields | reserved misc buffer not surfaced |
| Wi-Fi Preview | [wifi_signaling.py](./wifi_signaling.py), [wifi_receiver.py](./wifi_receiver.py), and [wifi_packets.py](./wifi_packets.py) | `rgb`, optional `intrinsics`, provenance device fields | decoded depth is held back until a real pose is available |
