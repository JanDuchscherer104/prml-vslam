...
parallel to the USB path in :mod:`prml_vslam.sources.record3d.record3d`, but its transport
...
class Record3DWiFiPreviewStreamSession:
    ...
    ``connect() -> wait_for_observation(...) -> disconnect()``. Internally the
    ...
    def __init__(self, config: Record3DWiFiPreviewStreamConfig) -> None:
        ...
        self._packet_queue: Queue[Observation] = Queue()
